...
main( int argc, char *argv[ ] )
{
        ...
        for( int times = 0; times < NUMTIMES; times++ )
        {
                ...
                #pragma omp parallel for default(none) shared(BeforeY, AfterY, DistX, RADIUS, stderr) reduction(+:numSuccesses)
                for( int n = 0; n < NUMTRIALS; n++ )
                {
			...
			float vx = sqrtf(2.* GRAVITY*(beforey-aftery));
			float t  = sqrtf((2. * aftery)/GRAVITY);
			float dx  = vx*t;
